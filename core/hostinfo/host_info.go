...
/* 引用 https://github.com/jiaocoll/BeeScan-web
创建人员：云深不知处
创建时间：2022/1/15
程序功能：获取本机信息
*/
...
func GetMemInfo() MemInfo {
	...
	utils.PrinfI("info", info)
	result := MemInfo{
		Percent: decimal(fmt.Sprintf("%.1f", info.UsedPercent)),
		All:     formatFileSize(info.Total),
		Used:    formatFileSize(info.Used),
		Fare:    formatFileSize(info.Free),
	}
	return result
}

func formatFileSize(fileSize uint64) string {
	return fmt.Sprintf("%.1f", float64(fileSize)/float64(GB))
}
