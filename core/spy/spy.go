...
func Start(e *run.Engine, Keyword string) {
	...
	if uri.IsIPv4(Keyword) {
		...
		HostDiscoveryIcmpPool(e, gatewayArr)
		return
	}
	...
	if Keyword == "all" || Keyword == "" || Keyword == "192" {
		...
		gatewayArr = uri.GetGatewayList("192.168", "b")
		gatewayArr = misc.RemoveDuplicateElement(gatewayArr, All...)
		if len(gatewayArr) > 0 {
			HostDiscoveryIcmpPool(e, gatewayArr)
			All = append(All, gatewayArr...)
	...
	if len(Keyword) > 4 {
		//探测常见网段192段，B段存活网关
		slog.Println(slog.INFO, "现在开始枚举常见网段"+Keyword)
		gatewayArr = uri.GetGatewayList(Keyword, "b")
		gatewayArr = misc.RemoveDuplicateElement(gatewayArr, All...)
		if len(gatewayArr) > 0 {
			HostDiscoveryIcmpPool(e, gatewayArr)
...
func HostDiscoveryIcmpPool(e *run.Engine, gatewayArr []string) {
	spyPool := pool.New(2000)
...
func pushTarget(ip string) {
	...
	for _, ip := range ipArr {
		ipStr := ip.String()
		slog.Println(slog.INFO, "pushTarget", ipStr)
