...
import (
	...
	"strconv"
	"strings"
	"zrWorker/lib/appfinger/iconhash"
...
func getIcon(URL url.URL, body string) string {
	...
	if regxIconPath.MatchString(path) == true {
		URL.Path = ""
		return GetIconMd5(URL.String() + path)
	}
	if regxIconURL.MatchString(path) == true {
		return GetIconMd5(path)
	}
	if regxIconURLNotProtocol.MatchString(path) == true {
		return GetIconMd5(URL.Scheme + path)
	}
	return ""
}
...
func GetIconMd5(URL string) string {
	resp, err := simplehttp.Get(URL)
	if err != nil {
		return ""
	}
	hash := utils2.Md5(resp.Raw.Body)
	return hash
}
