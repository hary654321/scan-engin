...
/*
解析 nmap-probes 中匹配规则字符串，转换成 golang 中可以进行编译的字符串
...
func (p *Probe) getDirectiveSyntax(data string) (directive Directive) {
	...
	if strings.Count(data, " ") <= 0 {
		slog.Println(slog.DEBUG, "nmap-probes - error directive format")
	}
...
func (p *Probe) parseProbeInfo(probeStr string) {
	...
	if !(proto == "TCP " || proto == "UDP ") {
		slog.Println(slog.DEBUG, "Probe <protocol>must be either TCP or UDP.")
	}
	if len(other) == 0 {
		slog.Println(slog.DEBUG, "nmap-probes - bad probe name")
	}
...
func (v *VScan) parseProbesFromContent(content string) {
	...
	// 判断第一行是否为 "Exclude " 设置
	if len(lines) == 0 {
		slog.Println(slog.DEBUG, "Failed to read nmap-probes file for probe data, 0 lines read.")
	}
	c := 0
	for _, line := range lines {
		...
		if c > 1 {
			slog.Println(slog.DEBUG, "Only 1 Exclude directive is allowed in the nmap-probes file")
		}
	}
	l := lines[0]
	if !(strings.HasPrefix(l, "Exclude ") || strings.HasPrefix(l, "Probe ")) {
		slog.Println(slog.DEBUG, "Parse error on nmap-probes file: line was expected to begin with \"Probe \" or \"Exclude \"")
	}
	if c == 1 {
...
var probesEmbed embed.FS

// 从文件中解析并加载 Probes 初始化 VScan 实例
func (v *VScan) Init() {

	fs, _ := probesEmbed.Open("nmap-service-probes.txt")

	sourceBuf, _ := io.ReadAll(fs)

	//println(string(sourceBuf))
	// 解析规则文本得到 Probe 列表
	v.parseProbesFromContent(string(sourceBuf))
...
func (v *VScan) Explore(target Target, config *Config) (Result, error) {
	...
	// 未进行特殊配置，默认只使用 NULL Probe 和包含了探测端口的 Probe 探针组
	{
		...
		probesUsed = append(probesUsed, v.ProbesMapKName["NULL"])
		probesUsed = append(probesUsed, v.ProbesMapKName["GetRequest"])
	}
	...
	//utils.WriteJson("probesUsed", probesUsed)
...
func (v *VScan) scanWithProbes(target Target, probes *[]Probe, config *Config) (Result, error) {
	...
	for _, probe := range *probes {
		//utils.WriteJson("probe.json", probe)
		var response []byte
...
func grabResponse(target Target, data []byte, config *Config) ([]byte, error) {
	...
	//slog.Println(slog.DEBUG, string(response))
	return response, nil
}
...
func (v *VScan) GoNmapScan(ip string, port string, protocol string) (*Result, error) {
